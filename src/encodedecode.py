...
class EncodeDecode(nn.Module):
    def __init__(self):
        ...
        self.cd1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(2048, 768, kernel_size=4, stride=2),
            nn.ReLU(inplace=True))

        self.cd2 = nn.Sequential(
            nn.ConvTranspose2d(1024, 384, kernel_size=4, stride=2),
            nn.ReLU(inplace=True)
            )

        self.cd3 = nn.Sequential(
            nn.ConvTranspose2d(512, 192, kernel_size=4, stride=2),
            nn.ReLU(inplace=True)
            )
    ...
    def forward(self, im1, im2):
        ...
        cd1ec5feature = torch.cat((cd1, ec5), dim=1)
        cd2 = self.cd2(cd1ec5feature)
        ec4feature = self.ec4feature(ec4)
        cd2ec4feature = torch.cat((cd2, ec4), dim=1)
