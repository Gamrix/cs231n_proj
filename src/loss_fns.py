...
class TextureLoss2(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        for i in range(1):
            dist = 2 ** 3
            text_loss += self.l2_loss(torch.abs(self.delta_x(y_pred, dist)),
                                      torch.abs(self.delta_x(y_true, dist)))
            text_loss += self.l2_loss(torch.abs(self.delta_y(y_pred, dist)),
                                      torch.abs(self.delta_y(y_true, dist)))
