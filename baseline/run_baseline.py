...
def make_loaders(inputs, gold):
    ...
    train = DataLoader(dataset, batch_size=16, sampler=ChunkSampler(NUM_TRAIN, 0))
    val = DataLoader(dataset, batch_size=16, sampler=ChunkSampler(NUM_VAL, NUM_TRAIN))
    test = None # For now
...
def run_model(train_data, val_data, test_data):
    model_base = nn.Sequential (
        # Conv 1
        nn.Conv2d(6, 32, kernel_size=3, stride=1, padding=(1,1)), # out 32 * 224 * 224
        nn.BatchNorm2d(32),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=DROPOUT),
        # Conv 2
        nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=(1,1)), # out 64 * 224 * 224
        nn.BatchNorm2d(64),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=DROPOUT),
        # Conv 3
        nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=(1,1)), # out 32 * 224 * 224
        nn.BatchNorm2d(32),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=DROPOUT),
        # Conv 4
        nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=(1,1)), # out 16 * 224 * 224
        nn.BatchNorm2d(16),
        nn.ReLU(inplace=True),
        nn.Dropout2d(p=DROPOUT),
        # Conv 5
        nn.Conv2d(16, 3, kernel_size=3, stride=1, padding=(1,1)), # out 3 * 224 * 224
        nn.BatchNorm2d(3),
        nn.ReLU(inplace=True),
    )
